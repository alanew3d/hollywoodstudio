...
export const AIService = {
  /**
   * Hollywood Studio AI credit rule.
   * For the Brazilian plans, keep it simple for the user:
   * 1 credit = 1 second of generated video.
   *
   * We keep the full signature to avoid breaking existing UI calls.
   */
  getCreditCost(mode, duration, quality, resolution) {
    const seconds = Number.parseInt(duration || 0, 10);
    return Math.max(1, seconds);
  },

  /**
   * Execute a generation request. Phase 1 keeps the original Seedance/muapi-compatible flow.
   */
  async generate(userId, { mode, prompt, aspect_ratio = "16:9", resolution = "720p", duration = 5, quality = "basic", images_list = [], video_files = [], audio_files = [] }) {
    ...
    const webhookPath = config.ai.provider === "muapi" ? "muapi" : config.ai.provider;
    const webhookUrl = `${config.auth.webhook_url}/api/webhook/${webhookPath}`;
