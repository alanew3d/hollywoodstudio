...
async function startCheckout(plan) {
  ...
  if (!window.S || !window.S.user || !window.S.user.uid) {
    if (typeof showToast === 'function') showToast('Faça login para assinar um plano.', 'warn');
  ...
  try {
    const resp = await fetch('/api/create-checkout-session', {
      ...
      body: JSON.stringify({
        plan,
        userId: window.S.user.uid,
        email:  window.S.user.email,
      }),
    });

    const data = await resp.json();
    if (!resp.ok || !data.url) throw new Error(data.error || 'Checkout indisponível. Verifique as configurações da Stripe.');
